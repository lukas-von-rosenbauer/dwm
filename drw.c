	 ...
	 int filled, int invert)
{
	...
	XSetForeground(drw->dpy, drw->gc,
		       invert ? drw->scheme[ColBg].pixel : drw->
		       scheme[ColFg].pixel);
	if (filled)
...
// Create a function that would allow to specify the color
// Instead of using a predefined scheme
//
// Old: 
// drw->scheme[ColBg]
//
// New:
// void draw_rect_color(... , int color, ...);
// ...
// drw->scheme[color]
	 ...
	 unsigned int lpad, const char *text, int invert)
{
	...
	while (1) {
		...
		if (!*text) {
			...
			fcpattern =
			    FcPatternDuplicate(drw->fonts->pattern);
			FcPatternAddCharSet(fcpattern, FC_CHARSET,
					    fccharset);
			FcPatternAddBool(fcpattern, FC_SCALABLE, FcTrue);
			FcPatternAddBool(fcpattern, FC_COLOR, FcFalse);

			FcConfigSubstitute(NULL, fcpattern,
					   FcMatchPattern);
