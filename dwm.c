...
struct Client {
	...
	int isfixed, isfloating, isurgent, neverfocus, oldstate,
	    isfullscreen, isterminal, noswallow, CenterThisWindow;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
void applyrules(Client * c)
{
	...
	if (ch.res_name)
		XFree(ch.res_name);
	c->tags =
	    c->tags & TAGMASK ? c->tags & TAGMASK : c->mon->tagset[c->mon->
								   seltags];
}
...
void drawbar(Monitor * m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i],
			 urg & 1 << i);
		if (occ & 1 << i)
			// Call draw_rect_color(... , int color, ...); 
			// with border color (check drw.h line 15 enum)
...
Atom getatomprop(Client * c, Atom prop)
{
	...
	if (XGetWindowProperty
	    (dpy, c->win, prop, 0L, sizeof atom, False, XA_ATOM, &da, &di,
	     &dl, &dl, &p) == Success && p) {
		atom = *(Atom *) p;
		XFree(p);
	}
...
void setfullscreen(Client * c, int fullscreen)
{
	if (fullscreen && !c->isfullscreen) {
		XChangeProperty(dpy, c->win, netatom[NetWMState], XA_ATOM,
		...
		XChangeProperty(dpy, c->win, netatom[NetWMState], XA_ATOM,
				32, PropModeReplace, (unsigned char *) 0,
				0);
		c->isfullscreen = 0;
...
void seturgent(Client * c, int urg)
{
	...
	if (!(wmh = XGetWMHints(dpy, c->win)))
		return;
	wmh->flags =
	    urg ? (wmh->flags | XUrgencyHint) : (wmh->
						 flags & ~XUrgencyHint);
...
{
	if (!gettextprop
	    (c->win, netatom[NetWMName], c->name, sizeof c->name))
...
pid_t winpid(Window w)
{
	...
	for (; i.rem; xcb_res_client_id_value_next(&i)) {
		...
		if (spec.mask & XCB_RES_CLIENT_ID_MASK_LOCAL_CLIENT_PID) {
			uint32_t *t =
			    xcb_res_client_id_value_value(i.data);
			result = *t;
			break;
		}
	}
	...
	if (XGetWindowProperty
	    (dpy, w, XInternAtom(dpy, "_NET_WM_PID", 0), 0, 1, False,
	     AnyPropertyType, &type, &format, &len, &bytes,
	     &prop) != Success || !prop)
		return 0;
